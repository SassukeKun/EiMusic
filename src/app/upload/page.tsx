...
export default function UploadPage() {
  ...
  const [uploadMode, setUploadMode] = useState<
    "select" | "single" | "album" | "video"
  >("select");

  // Estados para gerenciar o upload e formulário
  const [uploadStep, setUploadStep] = useState<
    "initial" | "details" | "success" | "error"
  >("initial");
  ...
  const handleCancelUpload = useCallback(async () => {
    try {
      console.log("Starting cancel upload process...");

      // Passo 1: Sinalizar que estamos resetando
      setIsResetting(true);
      setUploadCancelled(true);

      // Passo 2: Limpar todos os estados de arquivo
      setAudioFile(null);
      setAudioName("");
      setCoverImageFile(null);
      setCoverImagePreview(null);

      // Passo 3: Limpar todos os estados de formulário
      setTitle("");
      setGenre("");
      setMood("");
      setDescription("");
      ...
      // Passo 4: Limpar estados de resultado
      setUploadedResult(null);

      // Passo 5: Resetar os inputs de arquivo
      ...
      // Passo 6: Pequeno delay para garantir que tudo foi limpo
      await new Promise((resolve) => setTimeout(resolve, 100));

      // Passo 7: Resetar o estado de upload
      resetUploadState();

      // Passo 8: Finalizar o processo de reset
      setUploadCancelled(false);
      setIsResetting(false);

      // Passo 9: Voltar para o estado inicial
  ...
  // Função para reset completo do componente - NOVA FUNÇÃO
  const handleCompleteReset = useCallback(() => {
    console.log("Performing complete reset...");

    // Limpar todos os estados de arquivo
    setAudioFile(null);
    setAudioName("");
    setCoverImageFile(null);
    setCoverImagePreview(null);

    // Limpar todos os estados de formulário
    setTitle("");
    setGenre("");
    setMood("");
    setDescription("");
  ...
  //actions para upload em termos de reset e cancelamento
  ...
  useEffect(() => {
    ...
    // PROTEÇÃO 1: Não reagir durante reset ou cancelamento
    if (isResetting || uploadCancelled) {
      console.log("Ignoring state change during reset/cancel");
      return;
    }

    // PROTEÇÃO 2: Não processar se não há arquivo selecionado
    if (!audioFile) {
      console.log("No audio file selected, ignoring state change");
      return;
    }

    // PROTEÇÃO 3: Só processar se estivermos no fluxo correto
    if (uploadStep !== "details" && !uploadState.isUploading) {
      console.log("Not in upload flow, ignoring state change");
      return;
    }
    ...
    if (
      ...
      console.log("Upload completed successfully, transitioning to success");
      setUploadStep("success");
      return;
    }

    // Se ocorrer um erro durante o upload (e não foi cancelado)
    if (uploadState.error && !uploadCancelled) {
      console.log("Upload error detected:", uploadState.error);
      setUploadStep("error");
      return;
    }
  }, [
  ...
  if (uploadState.isUploading) {
    return (
      <div className="min-h-screen bg-gray-900 text-white flex items-center justify-center p-4">
        <div className="text-center max-w-lg w-full bg-gray-800/90 rounded-xl p-6 sm:p-8 shadow-2xl border border-gray-700 backdrop-blur-sm">
          {/* Cabeçalho com ícone animado */}
          <div className="relative mb-6">
            <motion.div
              className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-gradient-to-r from-indigo-600 to-purple-600 mb-4 relative overflow-hidden"
              animate={{
                scale: [1, 1.05, 1],
                boxShadow: [
                  "0 0 0 0 rgba(79, 70, 229, 0.4)",
                  "0 0 20px 10px rgba(79, 70, 229, 0.1)",
                  "0 0 0 0 rgba(79, 70, 229, 0.4)",
                ],
              }}
            ...
            >
              {/* Ícone principal */}
              <FaCloudUploadAlt className="text-3xl text-white relative z-10" />
          ...
          <p className="text-gray-300 mb-6 text-sm leading-relaxed">
            {uploadState.progress < 25 && "Preparando e validando arquivo..."}
            ...
            {uploadState.progress >= 75 &&
              ...
              "Processando e otimizando..."}
            {uploadState.progress >= 95 && "Finalizando upload..."}
          </p>
          ...
          <div className="relative w-full mb-8">
            <div className="relative w-full h-4 bg-gray-700 rounded-full overflow-hidden shadow-inner">
              {/* Barra de progresso principal */}
              <motion.div
                className="absolute top-0 left-0 h-full bg-gradient-to-r from-indigo-500 via-purple-500 to-indigo-600 rounded-full shadow-lg"
                initial={{ width: 0 }}
          ...
          {/* Informações do arquivo sendo enviado */}
          <div className="w-full bg-gray-700/40 rounded-lg p-4 mb-6 border border-gray-600/30">
            <div className="flex items-start space-x-3 mb-3">
              <div className="w-12 h-12 bg-indigo-600/20 rounded-lg flex items-center justify-center flex-shrink-0 border border-indigo-500/30">
                <FaMusic className="text-indigo-400 text-lg" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="mb-1">
                  <p className="font-medium text-white text-sm leading-tight">
                    <span className="block">
                      {(title || audioName)?.length > 40
                        ? (title || audioName).substring(0, 40) + "..."
                        : title || audioName}
                    </span>
                    {(title || audioName)?.length > 40 && (
                      <span className="block text-xs text-gray-300 mt-1">
                        {(title || audioName).substring(40, 80)}
                        {(title || audioName)?.length > 80 ? "..." : ""}
                      </span>
                    )}
                  </p>
                </div>
            ...
            {/* Informação da capa se houver */}
            {coverImageFile && (
              <div className="flex items-center pt-3 border-t border-gray-600/30">
                <div className="w-8 h-8 bg-purple-600/20 rounded-lg flex items-center justify-center mr-3 flex-shrink-0 border border-purple-500/30">
                  <FaImage className="text-purple-400 text-sm" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-gray-400">
                        {coverImageFile?.size
                          ? `${(coverImageFile.size / (1024 * 1024)).toFixed(
                              1
                            )} MB`
                          : ""}
                      </p>
            ...
            {uploadState.progress < 95 ? (
              <motion.button
                onClick={() => {
                  if (
                    window.confirm(
                      "Tem certeza que deseja cancelar o upload? O progresso será perdido."
                    )
                  ) {
                    handleCancelUpload();
                  }
                ...
                className={`w-full py-3 text-white font-medium rounded-lg 
                           transition-all duration-200 border backdrop-blur-sm shadow-lg
                           ${
                             isResetting
                               ? "bg-gray-600/80 border-gray-500/50 cursor-not-allowed"
                               : "bg-red-600/80 hover:bg-red-600 border-red-500/50 hover:border-red-400 hover:shadow-red-500/25"
                           }`}
              ...
              <div
                className="w-full py-3 bg-green-600/20 border border-green-500/50 text-green-300 font-medium rounded-lg 
          ...
          <div className="mt-6 p-3 bg-yellow-900/20 border border-yellow-700/50 rounded-lg">
            <p className="text-yellow-200 text-xs text-center leading-relaxed">
              ...
              Mantenha esta janela aberta até a conclusão do upload
            </p>
          </div>
        </div>
      </div>
  ...
  const handleFeaturedArtistsChange = (
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // IMPORTANTE: Resetar estados de controle antes de iniciar
    setUploadCancelled(false);
    setIsResetting(false);
    ...
    try {
      ...
      const audioMetadata: Omit<AudioMetadata, "uploadDate"> = {
        ...
        featuredArtists: featuredArtists.trim()
          ? featuredArtists.split(",").map((a) => a.trim())
          : undefined,
      };

      console.log("Metadata prepared:", audioMetadata);
      ...
      if (result && !uploadCancelled) {
        ...
        console.log(
          "Upload successful, useEffect will handle transition to success"
        );
        // Não mudamos o step aqui - deixamos o useEffect detectar o sucesso
      } else if (!uploadCancelled) {
        console.log("Upload failed, transitioning to error state");
        setUploadStep("error");
      } else {
      ...
      if (!uploadCancelled) {
        ...
        console.log("Upload error ignored due to cancellation");
      }
    }
  };
  ...
  const renderSuccessScreen = () => {
    // PROTEÇÃO: Verificar se realmente temos um resultado válido
    if (!uploadedResult && !title) {
      console.warn(
        "Success screen rendered without valid result, redirecting..."
      );
      setUploadStep("initial");
      return null;
    }
      ...
      >
        <div className="mb-8">
          ...
          <p className="text-gray-400 mb-4">
            Sua música foi enviada e está sendo processada. Em breve estará
            disponível na plataforma.
          </p>

          {/* Informações da música enviada - MELHORADO */}
          <div className="mt-6 p-4 bg-gray-700/50 rounded-lg inline-block text-left max-w-md">
    ...
    return (
      ...
      >
        <div className="mb-8">
          ...
          <p className="text-gray-400 mb-4">
            {uploadState.error ||
              "Ocorreu um erro ao fazer o upload da música. Por favor, tente novamente."}
          </p>

          {/* Informações de debug (somente em desenvolvimento) */}
          {process.env.NODE_ENV === "development" && uploadState.error && (
            <div className="mt-4 p-3 bg-gray-700/30 rounded-lg text-left text-xs">
              <p className="text-gray-300">
                <strong>Debug Info:</strong>
                <br />
                Error: {uploadState.error}
                <br />
                Progress: {uploadState.progress}%<br />
                File: {audioFile?.name || "None"}
                <br />
                Step: {uploadStep}
              </p>
            </div>
        ...
        <div className="flex flex-col sm:flex-row justify-center space-y-3 sm:space-y-0 sm:space-x-4">
          <motion.button
            onClick={() => {
              // Limpar erro e voltar para detalhes se ainda temos arquivo
              resetUploadState();
              if (audioFile) {
                setUploadStep("details");
              } else {
      ...
      <div className="min-h-screen bg-gray-900 text-white pb-20">
          ...
          <div className="max-w-5xl mx-auto px-4 py-6 relative overflow-hidden">
            ...
            <div className="relative flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <div className="flex items-center">
                  ...
                  <h1 className="text-3xl md:text-4xl font-bold ml-4 text-white">
                    Publique sua arte
                  </h1>
                </div>
              </div>
            ...
            {uploadStep === "details" && (
              <div className="absolute bottom-0 left-0 w-full h-1 bg-gray-800">
                <div className="h-full bg-gradient-to-r from-indigo-500 via-purple-500 to-indigo-500 rounded-full"></div>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="max-w-5xl mx-auto px-4 py-8">
          ...
          <p className="text-gray-400 mb-8">
            Transforme sua música em uma experiência para o mundo
          </p>

          {uploadStep === 'initial' ? (
          <div>
            {/* Botão de voltar para seleção de tipo - NOVO */}
            <div className="mb-6 flex justify-start">
              ...
              >
                <FaArrowLeft className="mr-2 group-hover:text-indigo-400" />
                <span className="text-sm">Voltar para opções de upload</span>
              </motion.button>
            </div>
                  ...
                  <div className="md:col-span-2 space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-gray-500 text-xs mt-1">
                        As tags ajudam a tornar sua música mais fácil de
                        encontrar
                      </p>
                    </div>
                  </div>
